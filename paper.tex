...
\begin{itemize}
    ...
    Otherwise, \(p_i \leq \min(p_{i_k}, \lambda_i - q_{i_k})\), where \(k = \max S\).\footnote{I could remove the edge cases if I define \(q_{i_{-1}}\) and \(q_{i_{r + 1}}\)...}
\end{itemize}
...
\begin{proof}
    ...
    Then we have \(q_{i_r} + p_{i_{r + 1}} > \lambda_i\).
    Then 
    \par Let \(k \in \{1, ..., r + 1\} \setminus S\).
    We have \(q_{i_{k - 1}} + p_{i_k} \leq \lambda_i\).
    Suppose we had \(q_i < q_{i_{k - 1}}\).
    Then \(p_i \leq \max_{j : q_j < q_{i_{k - 1}}} p_j = p_{i_k}\).
    But then \(p_i + q_i < q_{i_{k - 1}} + p_{i_k} \leq \lambda_i\), which is nonsense.


    \par There are a few cases: either \(q_i \geq q_{i_0}\), or there's some \(k\) with \(q_{i_{k - 1}} > q_i \geq q_{i_k}\), or else \(q_{i_r} > q_i\).
    In the case that \(q_i \geq q_{i_0}\) we have \(p_i \leq \lambda_i - q_{i_0}\).
    In the case that \(q_{i_{k - 1}} > q_i \geq q_{i_{k - 1}}\), we have \(p_i \leq \lambda_i - q_{i_k}\), and also \(p_i \leq \max_{j : q_j < q_{i_{k - 1}}} p_j = p_{i_k}\).
    That is, \(p_i \leq \min(\lambda_i - q_{i_k}, p_{i_k})\).
    In the case that \(q_{i_r} > q_i\), we have \(p_i \leq \max_{j : q_j < q_{i_r}} p_j = p_{i_{r + 1}}\).
    So,
    \[p_i \leq \max(\lambda_i - q_{i_0}, \max_k \min(\lambda_i - q_{i_k}, p_{i_k}), p_{i_{r + 1}})\]

    For \(s \in \{1, ..., r + 1\}\), we have two cases: either \(q_i < q_{i_s}\), or \(q_i \geq q_{i_s}\).
    

    then \(p_i \leq \max_{j : q_j < q_{i_s}} p_j = p_{i_{s + 1}}\).
    So, either \(\)
