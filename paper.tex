...
\begin{proof}
    ...
    For any \(P : U \to U\), let \(S_P := \#\{j : f(P(e_j)) \neq 0\}\).
    Choose \(P\) to minimize \(S_P\), and then let \(e_j' := P(e_j)\).
    Suppose, for contradiction, that \(S_P > 1\).
    Then let \(j_0 = \min\{j : f(e_j') \neq 0\}\), and let \(k\) be distinct from \(j\) with \(f(e_k') \neq 0\).
    Then make the change of basis \(e_{\lambda_{i_0}}'' := e_{\lambda_{i_0}}' + ...\).
\end{proof}
