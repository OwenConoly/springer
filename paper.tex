...
In this section we find the irreducible components of
\[\pi^{-1}(X) \cong V := \{(U, a, b) : \forall i. \; A_{X, a, b} U_{i + 1} \subseteq U_i\}.\]

\begin{lemma}
    \(V = V_0 \cup \bigcup_{w, r} V_{w,r}\), where 
    \[V_0 = \{(U, 0, b)\} \subseteq V,\]
    and
    \[V_{w,r} = \{(U, a, b) : \exists P \in \GL_{m}. \; \exists b'. \; (P^{-1}, I_n) A_{X, a, b} (P, I_n) = A_{X, e_{wr}, b'} \} \subseteq V.\]
\end{lemma}
\begin{proof}
    TODO
\end{proof}
Now, we will find the irreducible components of \(V_0\) and the \(V_{w,r}\).
These will all happen to be equidimensional, so we will see that their closures in \(V\) are the irreducible components of \(V\).
\par We begin with \(V_0\), which will be easy.
