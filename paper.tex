...
\begin{proof}
    ...
    Referring to those, we see that \(\dim \mathcal{Q}_{w,r,I,(\rho_i),\alpha}\) cancels with \(\dim G_{wr}\), and we are left with \(\dim G = \sum_{ij} \min(\lambda_i, \lambda_j)\).
\end{proof}
...
\begin{theorem}\label{springer_fiber_comps}
    ...
    Each irreducible component of \(\pi^{-1}(J(\lambda))\) has dimension \(\sum_{ij} \min(\lambda_i, \lambda_j) + \sum_{i \neq j} \min(\lambda_i, \lambda_j)\).
\end{theorem}
\begin{proof}
    ...
    Adding the dimensions gives the claimed result.
\end{proof}

Conjugating \(\pi^{-1}(J(\lambda))\) by \((centralizer of X) \times \{I_n\}\) gives \(m^2 - \sum_{ij} \min(\lambda_i,\lambda_j)\) extra dimensions.


the Springer fiber at the \(n\)-Slodowy slice at 
