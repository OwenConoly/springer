...
\begin{theorem}\label{pwr_comps}
    ...
    TODO: error propagated here from \cref{gwr_irred_dim}. fix.
\end{theorem}
\begin{remark}
...
\begin{proof}
    ...
    \[\sum_{i \leq j} \min(\lambda_i', \lambda_j') + \sum_{ij} \min(\lambda_i, \lambda_j) - \sum_{ij} \min(\lambda_i, \lambda_j').\]
    Now, \(\sum_{i \leq j} \min(\lambda_i', \lambda_j') + r \#\{i : \lambda_i \geq= \sum_{i \leq j} \min(\lambda_i, \lambda_j)\)
    something something cancels out.
    see that \(\dim \mathcal{Q}_{w,r,I,(\rho_i),\alpha}\) cancels with \(\dim G_{wr}\), and we are left with \(\dim G = \sum_{ij} \min(\lambda_i, \lambda_j)\).
\end{proof}
...
\begin{corollary}\label{p_comps}
    ...
    Each has dimension \(\sum_{ij} \min(\lambda_i, \lambda_j)\).
    TODO: error propagated here from \cref{pwr_comps}. fix.
\end{corollary}
\begin{proof}
...
\begin{theorem}\label{springer_fiber_comps}
    ...
    TODO: error propagated here from \cref{p_comps}.
\end{theorem}
\begin{proof}
...
\begin{theorem}
    Components of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) are blah.
\end{theorem}
\begin{proof}
