...
\subsection{Study of \(B_{I, (\rho_i)}\)}
...
\subsection{Study of \(\mathcal{Q}_{I, (\rho_i)}\)}
Fix any \(I\) and \((\rho_i)\) satisfying the conditions (1)--(4) of \cref{bs_union}.
...
\end{theorem}
\begin{proof}
    We see from \cref{u_i_rho_irred_and_dim} that the \(\mathcal{Q}_{I, (\rho_i), \alpha}\) are irreducible and equidimensional.
    Then \cref{bs_union} tells us that their union is \(\mathcal{Q}\), and in addition that none is contained in the union of the others.
\end{proof}

\section{The components of \(\mathcal{P}_{J(\lambda)}\)}
...
\subsection{The components of \(\mathcal{Q}_{wr}\)}
...
\begin{theorem}
    The irreducible components of \(\mathcal{Q}_{wr}\) are exactly the subvarieties \(\mathcal{Q}_{w,r,I,(\rho_i),\alpha}\).
    Each has dimension \(\sum_{ij} \min(\lambda_i', \lambda_j')\), where \(\lambda' = (\lambda_1, ..., \lambda_{w - 1}, \lambda_w - r, \lambda_{w + 1}, ..., \lambda_k)\).
\end{theorem}
\begin{proof}
...
\subsection{The varieties \(G_{wr}\) and \(G\)}
Fix \(w,r\).
...
\begin{proof}
    ...
    \par In the case that \(r > 0\), we appeal to \cref{nilpotent_centralizer}, which tells us exactly what \(\mathfrak{z}_{\gl_m}(X)\) looks like.
    Note that \(V = \{Y + I : Y \in \mathfrak{z}_{\gl_m}(X), e_{wr}Y = 0\}\).
    The constraint \(e_{wr}Y = 0\) is just saying that a certain row of \(Y\) must be all zeroes.
    Looking at the form of \(\mathfrak{z}_{\gl_m}(X)\), we see that requiring this row equal to zero for \(M \in \mathfrak{z}_{\gl_m}(X)\) is just the statement that \(M_{wr}\) is of the form
    \[\]

    From looking at the form of \(\mathfrak{z}_{\gl_m}(X)\) given in \cref{nilpotent_centralizer}, it is clear that 
    Lemma TODO says that \(V\) is isomorphic to \(\mathbb{C}^{\sum_{ij} \min(\lambda_i', \lambda_j')}\).
\end{proof}

\subsection{The components of \(\mathcal{P}_{wr}\)}
...
\begin{theorem}\label{pwr_comps}
    The irreducible components of \(\mathcal{P}_{wr}\) are the closures of the images \(\varphi_{wr}(\mathcal{Q}_{w,r,I,(\rho_i),\alpha} \times G)\).
    Some irreduble components of \(\mathcal{P}_{wr}\) are double-counted; we have equality iff there's a bijection \(i \leftrightarrow i'\) with \(\rho_i = \rho'_{i'}\) and \(\lambda_i = \lambda_{i'}\).
    When there is no equality, none is in the union of the others.
    Each has dimension blah.
\end{theorem}
\begin{proof}
    TODO: fix statement and proof here.
    Together, \cref{Q_comps} and \cref{g_irred_dim} tell us that \(\mathcal{Q}_{w,r,I,(\rho_i),\alpha}\) is irreducible, and therefore its image is irreducible.
    They are equidimensional, as we will verify in a moment.
    So, to verify that their closures are the irreducible components of \(\mathcal{P}_{wr}\), we need only check that none is contained in the union of the others.
\end{proof}

\subsection{The components of \(\mathcal{P}\)}
\begin{corollary}
    They are just the components of the \(\mathcal{P}_{wr}\)'s.
\end{corollary}
...
\begin{lemma}
    ...
    Let \(i = [\sum_{j < w} \lambda_j] + (r - 1)\).
    The set of \(M \in \mathfrak{z}_{\gl_m}(J(\lambda))\) such that \(\mathbbm{1}_iM = \mathbbm{1}_i\) is  
\end{lemma}
