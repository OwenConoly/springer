...
\begin{lemma}
    \(U_0 = \bigcup_{I', (\rho_i')} U_{I', (\rho_i')}\), where \(I'\) ranges over all possible subsets of \(\{1, ..., k\}\), and \((\rho_i')\) ranges over all possible maps \(I' \to \mathbb{N}_{>0}\) such that the maps \(i \mapsto \rho_i'\) and \(i \mapsto \lambda'_i - \rho_i'\) are both decreasing with \(i\).
\end{lemma}
\begin{proof}
    Let \((A_{0, b}, U) \in U_0\).
    Define \(I := \{i : p_i > \max_{j > i} p_j\}\), and let \(I' := \{n_i : i \in I\}\).
    And for \(i \in I\), \(\rho_{n_i}' := p_i\).
    ...
    \par For \(i, j \in I\) with \(i < j\), we have \(q_i \geq q_j\) (as \(q\) is nonincreasing), and also \(p_i > p_j\) (as \(i \in I\), and consequently \(p_i > \max_{l > i} p_l\)).
    Thus, \(\lambda_i > \lambda_j\).
