...
\begin{theorem}\label{pwr_comps}
    ...
    Each has dimension \(\sum_{ij} \min(\lambda_i, \lambda_j)\).
\end{theorem}
\begin{remark}
    ...
    Some more-careful analysis is required to determine which ones are distinct.
\end{remark}
\begin{proof}
    ...
    Indeed, since \(\varphi_{wr}\) is a principal \(G_{wr}\)-bundle by \cref{principal_bundle}, we infer that the dimension of \(\varphi_{wr}(\mathcal{Q}_{w,r,I,(\rho_i),\alpha} \times G)\) is \(\dim \mathcal{Q}_{w,r,I,(\rho_i),\alpha} + \dim G - \dim G_{wr}\).
    We calculated these dimensions in \cref{qwr_comps}, \cref{g_irred_dim}, \cref{gwr_irred_dim}.
    We see that \(\dim \mathcal{Q}_{w,r,I,(\rho_i),\alpha}\) cancels with \(\dim G_{wr}\), and we are left with \(\dim G = \sum_{ij} \min(\lambda_i, \lambda_j)\).
    \par TODO: can we conclude the difference-of-dimensions thing when the preimage isn't closed under action by \(G_{wr}\)?
    Surely not... so this is all wrong.
    I need to find out what happens when you let \(G_{wr}\) act on \(\mathcal{Q}_{w,r,I,(\rho_i),\alpha}\).
\end{proof}
