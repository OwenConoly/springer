...
\begin{proof}
    ...
    Since \(A_{a, b}\) is nilpotent, \cref{normalization} says that there is a change of basis \(P\) such that \(P^{-1}XP = X\), and for all but one pair \((i, j)\) we have \(f(Pe_{ij}) = 0\).
    Since \(P^{-1}XP = X\), it is easy to see (writing out the matrix multiplication, if necessary) that there are \(a',b'\) such that \((P^{-1}, I_n)A_{a,b}(P, I_n) = A_{a',b'}\).
    \par Now,
    \begin{align*}
        P^{-1}XPe_{ij} + f(Pe_{ij}) f_n & = \\
        (P^{-1}, I_n)A_{a,b}(P, I_n)(e_{ij}, 0) & = \\
        A_{a'b'} (e_{ij},0) & = \\
        Xe_{ij} + a'_{ij} f_n & .
    \end{align*}
    Thus we see that \(a'_{ij} = 0\) for all pairs \((i,j)\) except one; or in other words, \(A_{a'b'}\) is of the form \(A_{e_{wr}b'}\).
\end{proof}
\begin{lemma}
