...
\maketitle
\begin{abstract}
    Let \(e\), be a regular nilpotent element of \(\gl_n\), \(n \geq 1\).
    We consider the Slodowy slice \(S\) at the element \((0, e) \in \gl_m \times \gl_n \subseteq \gl_{m + n}\).
    We define the \emph{Springer fiber of S} at a nilpotent \(X \in \gl_m\) as consisting of the following data: some \(Y \in S\), along with an element of the usual Springer fiber at \((X, Y) \in \gl_m \times \gl_{m + n}\).
    ...
    and use this to study another variety.
\end{abstract}
...
\par Before talking about future work, say what is the main result of the paper.

\subsection{Future Work}
We describe a possible extension to this work and an application to representation theory.
...
For nice (TODO: how nice?) \(\mathfrak{h}\), there is a correspondence between the Weyl group of \(\mathfrak{h}\) and irreducible components of \(\St_\mathfrak{h}\)\cite{zhiweiyun}[1.5.10].
\par There is a nice bijection between \(\St_{\gl_m}\) and the set \(T_m = \{(t_1, t_2) : \lambda \vdash m; t_1, t_2 \in \mathrm{SYT}(\lambda)\}\).
(Note that \(\mathrm{SYT}(\lambda)\) denotes the set of standard Young tableaux of shape \(\lambda\).)
One way to obtain this bijection is as follows.
First take the aforementioned correspondence between \(\St_{\gl_m}\) and the Weyl group of \(\gl_m\)---the Weyl group of \(\gl_m\) is the symmetric group \(S_m\).
Then take the Robinson-Schensted-Knuth correspondence, which is a bijection between \(S_m\) and \(T_m\).
More details about the the correspondence \(\St_{\gl_m} \leftrightarrow T_m\) can be found in \cite{wolff}.
\par An element of \(\St_{\gl_m}\) consists of a triple \((X, \mathfrak{b}_1, \mathfrak{b}_2)\), where \(\mathfrak{b}_1\) and \(\mathfrak{b}_2\) are elements of the Springer fiber at \(X\).
When the Jordan form of \(X\) has shape \(\lambda\), the correspondence sends this triple to a pair \((t_1, t_2)\), where both are of shape \(\lambda\).
\par Somewhat similarly, we will show that an element of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) consists of a tuple \((X, Y, \mathfrak{b}_1, \mathfrak{b}_2)\), where \(X \in \mathcal{N}_m, Y \in \mathcal{N}_{m + n}\), and \(\mathfrak{b}_1, \mathfrak{b}_2\) are elements of the Springer fiber at \(X, Y\) respectively.
(Some nontrivial relation will hold between \(X\) and \(Y\), but we ignore that for now.)
For any given relation \(R \subseteq \{\lambda : \lambda \vdash m\} \times \{\mu : \mu \vdash (m + n)\}\), we can define 
\[T_{m,n} = \{(t_1, t_2) : \lambda \vdash m; \mu \vdash (m + n); t_1 \in \mathrm{SYT}(\lambda); t_2 \in \mathrm{SYT}(\mu); R(\lambda, \mu)\}.\]
We conjecture that there is a correspondence, analogous to \(\St_{\gl_m} \leftrightarrow T_m\), between \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) and \(T_{m,n}\) (for some appropriate choice of the relation \(R\)).
An extension of this project would determine the details of this correspondence.
\par Given \(C \in \Irr(\St_{\gl_m})\), we can first map it to the corresponding \((t_1, t_2) \in T_m\), and then obtain an element of \(\End(\mathbb{C}[\mathrm{SYT}(\lambda)])\) by taking the map which sends \(t_1 \mapsto t_2\) and \(t' \mapsto 0\) for \(t' \in \mathrm{SYT}(\lambda) \setminus \{t_1\}\).
In this way, we obtain a \(\mathbb{C}\)-algebra isomorphism
\[\mathbb{C}[\Irr(\widetilde{\mathcal{N}} \times_\mathfrak{g} \widetilde{\mathcal{N}})] \cong \bigoplus_{\lambda \vdash m} \End(\mathbb{C}[\mathrm{SYT}(\lambda))).\]
This is an instance of the fact that for any finite group \(G\), we have \(\mathbb{C}[G] \cong \bigoplus_{V \in \irrrep(G)} \End(\mathbb{C}[\mathrm{SYT}(\lambda)])\).
\par If we found a correspondence \(S \times_\mathfrak{g} \widetilde{\mathcal{N}} \leftrightarrow T_{m,n}\), this could give an analogous isomorphism 
...
which would be an isomorphism of \(\mathbb{C}[S_m]\)- or \(\mathbb{C}[S_{m + n}]\)-modules instead of \(\mathbb{C}\)-algebras.
...
% \par irreducble components have module structure of group algebra of Sn.
% has applications to representation theory.
% write about this here.
% The paper (TODO: cite) describes a bijection between the Steinberg variety (which is blah) and pairs of standard Young tableaux (each having shape blah).
% The variety \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) looks something like the Steinberg variety.
% Maybe there is a similar correspondence, between elements of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) and pairs of standard Young tableaux: one having the shape of the Jordan form of \(X\), one having the shape of the Jordan form of \(A_{X, a, b}\).
...
TODO.  UROP+ program and Haoshuo.
MIT (provided funding)?
...
\[\begin{pmatrix}
    ...
    & & J_{\lambda_k}
\end{pmatrix}.\]
A partition is always indexed in nonincreasing order, even if it is defined differently.
For example, if \(\mu = (4, 6, 3)\), then \(\mu_1 = 6, \mu_2 = 4, \mu_3 = 3\).
...
\[\mathcal{F}_X = \{gHg^{-1} : X \in gHg^{-1}\}.\]
% ^ use that notation everywhere for springer fiber
\begin{definition}
...
% \section{Future work}
% \par Separately, this paper would benefit from an introductory section where we motivate the study of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\).
