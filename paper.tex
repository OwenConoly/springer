...
\par For \(1 \leq w \leq \ell\) and \(0 \leq r \leq \lambda_w - 1\), define \(\mathcal{P}_{w,r}\) as the set of \((A_{a,b}, V_\bullet) \in \mathcal{P}\) such that there exist \(P \in \GL_m\) and \(b' \in \mathbb{C}^m\) such that \((P^{-1}, I_n) A_{a, b} (P, I_n) = A_{e_{w,r + 1}, b'}\).
Also, for \(1 \leq w \leq \ell\), define \(\mathcal{P}_{w,\lambda_w}\) as the set of \((A_{a,b}, V_\bullet) \in \mathcal{P}\) such that there exist \(P \in \GL_m\) and \(b' \in \mathbb{C}^m\) such that \((P^{-1}, I_n)A_{a,b}(P,I_n) = A_{0,b'}\).
\par The point of the definition above is that \((A_{a,b}, V_\bullet) \in \mathcal{P}_{wr}\) if and only if we can change basis of \(\mathbb{C}^m\) to get from \(A_{a,b}\) to some \(A_{a',b'}\) such that (1) \(a'_{ij} = 0\) for \(i \neq w\), and (2) \(a_{wj}' = 0\) for all but (at most) one \(j\), and (3) \(r\) is the biggest number s.t. \(a_{w1}' = a_{w2}' = \cdots = a_{wr}' = 0\).
...
\begin{proof}
    ...
    \par Since \((Pe_{ij})_{ij}\) is a Jordan basis, we have \((P^{-1}, I_n) A_{a,b}(P, I_n) = A_{a',b'}\) for some \(a',b' \in \mathbb{C}^m\).
    All that is left is to show that \(a'_{ij} = 0\) for all but one pair \((i,j)\).
    Indeed, this results from the fact that \(\phi_{a'}(e_{ij}) = \phi_a(Pe_{ij}) = 0\) for all but one pair \((i,j)\).
\end{proof}
\begin{lemma}
    \(\mathcal{P}_{w_1r_1} = \mathcal{P}_{w_2r_2}\) exactly when either \(\lambda_{w_1} - r_1 = \lambda_{w_2} - r_2 = 0\), or \((\lambda_{w_1}, r_1) = (\lambda_{w_2}, r_2)\).
\end{lemma}
\begin{proof}
    If \(r_1 = \lambda_{w1}\) and \(r_2 = \lambda_{w_2}\), then by definition, \(\mathcal{P}_{wr_1} = \mathcal{P}_{wr_2}\).
    ...
    Let \(k \geq 0\) be minimal such that there exists \(v \in \mathbb{C}^m\) with \(A_{e_{w_1r_1},0} f_1 \neq 0\).
    Obviously \(k = r_1\).
