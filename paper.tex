...
\par For any \(I \subseteq \{1, ..., k\}\) and any map \((i \mapsto \rho_i) : I \to \mathbb{N}_{> 0}\) such that (1) \(\rho_i \leq \lambda_i\), (2) \(\rho_i\) is decreasing with \(i\), (3) \(\lambda_i - \rho_i\) is decreasing with \(i\), and (4) \(\rho_i < n\), define \(B_{I, (\rho_i)}\) as the set of \(b \in \mathbb{C}^m\) satisfying the following conditions.
\begin{itemize}
    \item For all \(i \in I\), \(p_i = \rho_i\)
    \item For all \(i \notin I\), one of the following holds.
    \begin{itemize}
        \item \(q_i \geq n\)
        \item There is an \(i' \in I\) such that \(q_{i'} \leq q_i\) and \(p_{i'} \geq p_i\)
        \item \(p_i = 0\)
    \end{itemize}
\end{itemize}
...
\begin{lemma}\label{bs_fact}
    For any \(b \in B_{I, (\rho_i)}\) and any \(k \in \{0, ..., r\}\), we have \(p_{i_{k + 1}} = \max_{i : q_i < q_{i_k}} p_i\).
    \begin{itemize}
        \item 
    \end{itemize}
\end{lemma}
To see why this choice makes some sort of sense, look at the shape \(\mu\) of the Jordan basis of the previous subsection.
\par First, we provide an alternative characterization of \(B_{I, (\rho_i)}\).
\begin{lemma}\label{alternative_bs}
    \(B_{I, (\rho_i)} = B_{I, (\rho_i)}'\), where
    \[B_{I, (\rho_i)}' := \{b \in \mathbb{C}^m : \forall i \in I. \; p_i \leq \rho_i \land \forall i \notin I. \; p_i = \min(p_{i_k}, \lambda_i - q_{i_k}), \]
    \[\textrm{ where } k = \min\{k \in \{1, ..., r\} : q_{i_{k - 1}} + p_{i_k} > \lambda_i\}\}.\]
\end{lemma}
\begin{proof}
    First we show that \(B_{I, (\rho_i)} \subseteq B_{I, (\rho_i)}'\).
    Let \(b \in B_{I, (\rho_i)}\).
    It is clear that \(\forall i \in I. \; p_i = \rho_i\).
    So, let \(i \notin I\), and let \(k = \min\{k : q_{i_{k - 1}} + p_{i_k} > \lambda_i\}\).
