...
Then, we obtain \(p : S \to \mathfrak{g}\) by \(x \mapsto (x, p'(x))\).
...
As a step towards studying \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\), we consider the map \(\pi : S \times_\mathfrak{g} \widetilde{\mathcal{N}} \to \mathcal{N}_m\) given by taking the map to \(\mathfrak{g}\) and then projecting to \(\gl_m\).
...
There is a bijective correspondence between the Weyl group of \(\gl_m\) and irreducible components of \(\St_\mathfrak{\gl_m}\)(see \cite{ehf}[3.6] for details).
...
One way to obtain such a bijection is as follows.
First take the aforementioned correspondence between \(\St_{\gl_m}\) and the Weyl group \(S_m\) of \(\gl_m\).
...
\par An element of \(\St_{\gl_m}\) is a triple \((X, \mathfrak{b}_1, \mathfrak{b}_2)\), where \(\mathfrak{b}_1\) and \(\mathfrak{b}_2\) are elements of the Springer fiber at \(X\).
...
\par Somewhat similarly, we will show that an element of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\) is a subset of the set of tuples \((X, Y, \mathfrak{b}_1, \mathfrak{b}_2)\), where \(X \in \mathcal{N}_m, Y \in \mathcal{N}_{m + n}\), and \(\mathfrak{b}_1, \mathfrak{b}_2\) are elements of the Springer fiber at \(X, Y\) respectively.
...
\par Given an irreducible component \(C \in \Irr(\St_{\gl_m})\), we can first map it to the corresponding \((t_1, t_2) \in T_m\), and then obtain an element of \(\End(\mathbb{C}[\mathrm{SYT}(\lambda)])\) by taking the map which sends \(t_1 \mapsto t_2\) and \(t' \mapsto 0\) for \(t' \in \mathrm{SYT}(\lambda) \setminus \{t_1\}\).
...
\cref{sxn_comps} applies the results of section 7 to find the irreducible components of \(S \times_\mathfrak{g} \widetilde{\mathcal{N}}\).
Finally, \cref{linalg} proves some linear algebra lemmas that were used in the paper.

\subsection{Acknowledgments}
Thank you to Haoshuo Fu for suggesting the fun project, guiding me through it, and helping me to learn many things, both by talking to me about them and by suggesting things to read.
...
Also, we will write \(f_n, ..., f_1\) to denote the basis of \(\mathbb{C}^n\) with the property that \(J_n f_i = f_{i - 1}\).
...
Let \(\mathcal{N}_m\) be the nilpotent cone in \(\gl_m\), and let \(\mathcal{B}_m\) the variety of Borel subalgebras of \(\gl_m\).
...
Note that for \(X \in \gl_m\), we have [\(X \in \mathcal{N}_m\), and \(X\) preserves \(V_\bullet\)] if and only if [for all \(i\), \(XV_{i + 1} \subseteq V_i\)].
...
(\(\GL_n\) acts on the set of flags via \((g \cdot V_\bullet)_j = gV_j\).)
...
\[\mathcal{F}_X = \{g \cdot E_\bullet : X \in gHg^{-1}\} = \{V_\bullet : \forall i. \; XV_i \subseteq V_{i - 1}\}.\]
...
\section{Principal \texorpdfstring{\(\spl_2\)}{sl\_2}-triple in \texorpdfstring{\(\gl_n\)}{gl\_n}}\label{principal_sl2}
