...
Agh wait, this doesn't work out with my \(\rho_i\)'s...
\par For any \(I \subseteq \{1, ..., k\}\) and any values \((\rho_i)_{i \in I'}\) such that (1) \(\rho_i \leq \lambda_i\) and (2) \(\rho_{i_\alpha}\) is decreasing with \(\alpha\), define 
\[U_{I', (\rho_i)} := \{(A_{0, b}, U) \in U_0 : I' = \{i : p_i > \max_{j : q_j > q_i} p_j\} \land \forall i \in I'. \; p_i = \rho_i \}.\]
\begin{lemma}
...
\begin{proof}
    Let \((A_{0, b}, U) \in U_0\).
    Define \(I' = \{i : p_i > \max_{j : q_j > q_i} p_j\}\), and for \(i \in I'\), \(\rho_i := p_i\).
    ...
    \par Recall that by convention we take that the maximum of the empty set is zero.
    So, the fact that \(\rho_i > \max_{j : q_j > q_i} p_j\) tells us that \(\rho_i\) is positive.
    Now, let \(i < i'\), with \(i, i' \in I'\).
