...
\begin{itemize}
    \item For all \(k \in \{1, ..., r\}\), \(p_i = \rho_i\).
    \item For \(i \notin I\), let \(S = \{k \in \{1, ..., r + 1\} : q_{i_{k - 1}} + p_{i_k} > \lambda_i\}\).
    If \(S = \emptyset\), then \(p_i \leq \lambda_i - q_{i_0}\).
    If \(r + 1 \in S\), then \(p_i \leq p_{i_{r + 1}}\).
    Otherwise, \(p_i \leq \min(p_{i_k}, \lambda_i - q_{i_k})\), where \(k = \max S\).\footnote{I could remove the edge cases if I define \(q_{i_{-1}}\) and \(q_{i_{r + 1}}\)... ah, maybe a bit less drastically, I can say something like min(complement of S) instead of max(S)}
\end{itemize}
...
\begin{proof}
    ...
    Then if we had \(q_i < q_{i_0}\), this would entail \(p_i \leq \max_{j : q_j < q_{i_0}} = p_{i_1}\).
    ...
    But then \(p_i + q_i < q_{i_k} + p_{i_{k + 1}} \leq \lambda_i\), which is a contradiction since \(k + 1 \notin S\).
    Thus \(q_i \geq q_{i_k}\).
