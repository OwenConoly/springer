...
\begin{lemma}
    The map \(\varphi\) is a topological quotient of \(U_{wr} \times G\) by the action of the algebraic group \(G_{wr}\).
\end{lemma}
\begin{proof}
    TODO.
    See \url{http://www-fourier.ujf-grenoble.fr/~mbrion/notes_luminy.pdf}, definition 1.18.
\end{proof}
Now we only have to find the irreducible components of \(U_{wr} \times G\)

\begin{lemma}
    Let \(p : X \to Y\) be a topological quotient.
    Then the irreducible components of \(Y\) are the images of those of \(X\).
\end{lemma}
\begin{proof}
    Let \(C_i\) be the irreducible components of \(X\).
\end{proof}
