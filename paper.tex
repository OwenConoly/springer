...
\end{itemize}
For any \(b \in B_{I, (\rho_i)}\) we have \(p_{i_1} > \cdots > p_{i_r} > p_{i_{r + 1}}\), and also \(q_{i_0} > q_{i_1} > \cdots > q_{i_r}\).
...
\begin{lemma}
    ...
    \begin{itemize}
        ...
        \item For \(k \in \{1, ..., r\}\), the chain of length \(q_{i_k} + p_{i_{k + 1}}\) beginning with \(v_{i_k} - (A^{q_{i_k} + p_{i_{k + 1}}} v_{i_k} \rightshift q_{i_k} + p_{i_{k + 1}})\), where \(v_{i_k} := A^{n - q_{i_k}} f_n - \sum_{j = 1}^{p_{i_k}} b_{i_kj} e_{i,j + q_{i_k}}\)
    \end{itemize}
\end{lemma}
\begin{proof}
    ...
    \begin{proof}[Proof of (2)]
        ...
        \[\sum_{i \notin I} \lambda_i + \sum_{k = 0}^r (q_{i_k} + p_{i_{k + 1}}) = n + \sum_i (q_i + p_i) = m + n.\]
    \end{proof}
...
\begin{proof}
    ...
    Let \(L'\) be the result of taking \(\mu\) and, for each \(k\), replacing one occurence of \(q_{i_k} + p_{i_{k + 1}}\) by \(q_{i_k} + p_{i_k}\); that is,
    ...
    \par Let \(L = [q_{i_0}, \lambda_1, ..., \lambda_k]\).
