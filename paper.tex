...
\end{lemma}
\begin{proof}
    There are three things to check: (1) the chains are no longer than their claimed lengths, (2) the claimed lengths sum to \(m\), and (3) the span of the chains is \(\mathbb{C}^m\).
    ...
    \end{proof}
\end{proof}
\par We list the chains of the Jordan basis.
First, for each \(i \notin I\), we take the chain of length \(p_i + q_i\) beginning with \(e_{i,p_i + q_i} = e_{i\lambda_i}\).
