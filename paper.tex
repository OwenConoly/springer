...
The dimension of \(u'_{ij}\) is \(\min(\lambda_i, \lambda_j)\) in the case that \(i \neq i_0\), and \(\min(\lambda_i - j_0, \lambda_j)\) when \(i = i_0\).
Write \(d_{ij}\) for this dimension.
For each \(i\), let \(q_i\) be the length of the longest prefix of \(b_i\) which consists of zeroes.
Then if \(A_{X, a, b}\) is nilpotent, we must have \(q_{i_0} \geq j_0\).
So we may as well set \(q_{i_0} := q_{i_0} - j_0\).
\par Fix an \(i\).
I claim the rank of the matrix of \(b\)'s is \(h := \max_j [d_{ij} - q_j]\), or zero if this quantity is always negative.
First, to see that the rank is at least this big, note that for any \(j \in \argmax_j [d_{ij} - q_j]\), the rank of the \(j\)th ``column block'' of the matrix of \(b\)'s is equal to \(h\).
Then, to see that the rank is at most this big, note that every row after the first \(h\) rows is just zeroes.
So, the total degrees of freedom for the \(u_{ij}\) is 
\[\sum_j d_{ij} - \max(0, \max_j [d_{ij} - q_j]).\]
Then summing over all the \(i\), we get the dimension of \(C_3\):
\[\sum_i \left[\sum_j d_{ij} -\max(0, \max_j [d_{ij} - q_j])\right] = \sum_{ij} d_{ij} - \sum_i \max(0, \max_j[d_{ij} - q_j]).\]
When \(i \neq i_0\), then \(d_{ii} = \lambda_i \geq q_i\), and therefore \(\max(0, \max_j [d_{ij} - q_j]) = \max_j [d_{ij} - q_j]\).
When \(i = i_0\), then \(d_{ii} = \lambda_i - j_0 \geq q_{i_0}\).
So, we can simplify the above expression:
\[\sum_{ij} d_{ij} - \sum_i \max_j [d_{ij} - q_j].\]
Note that \(\max_j [d_{ij} - q_j] = \max_j [\min(p_i + q_i, p_j + q_j) - q_j]\).
This thing being maximized equals \(\min(p_i + q_i - q_j, p_j)\).
It would be really helpful if I could simplify this to \(p_j\) or something.
...
\par Now, we want to show that this chain is linearly independent from the chains we have already listed.
Because \(A^{r + n + P} e_{i_0, \lambda_{i_0}}\) is in the span of the chains we have already listed, it suffices to show that \(A^k e_{i_0, \lambda_{i_0}}\) is linearly independent of the previous chains, for all \(k < r + n + P\).
Indeed, by the definition of \(P\), we have that \(A^k e_{i_0, \lambda_{i_0}}\) has a nonzero \(e_{ij}\)-component, for some \(i\) with \(q_i < n + r\); and the chains we have listed previously all have \(e_{ij}\)-component equal to zero for \(i\) with \(q_i < n + r\).
...
To see that this is linearly independent of what came before... TODO.
